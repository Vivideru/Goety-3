    ...
    static boolean regrowNether(LevelAccessor accessor, BlockPos blockPos, BlockState originalState) {
        ...
        if (accessor instanceof Level level) {
            if (BlockFinder.canBeReplaced(level, blockPos) && !originalState.isAir() && !(originalState.getBlock() instanceof LiquidBlock) && !(originalState.getBlock() instanceof BaseFireBlock)) {
                if ((level.random.nextFloat() <= 0.15F && !(originalState.getBlock() instanceof SpecialPlantable)) || (originalState.getBlock() instanceof SpecialPlantable && Blocks.NETHERRACK.defaultBlockState().canSustainPlant(accessor, blockPos.below(), Direction.UP, originalState).isFalse())){
        ...
        if (originalState.is(Blocks.WATER)){
            ...
            accessor.setBlock(blockPos, blockstate, 3);
            accessor.levelEvent(2001, blockPos, Block.getId(blockstate));
            if (above.getBlock() instanceof SpecialPlantable && blockstate.canSustainPlant(accessor, blockPos, Direction.UP, above).isFalse()){
            ...
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(Tags.Blocks.STONES)) {
            BlockState blockstate = Blocks.BLACKSTONE.defaultBlockState();
            accessor.setBlock(blockPos, blockstate, 3);
            accessor.levelEvent(2001, blockPos, Block.getId(blockstate));
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(Blocks.MUD) || originalState.is(Tags.Blocks.SANDS)) {
            ...
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(Tags.Blocks.SANDSTONE_BLOCKS)) {
