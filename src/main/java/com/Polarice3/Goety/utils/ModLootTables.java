...
public class ModLootTables {
    ...
    public static final ResourceLocation EMPTY = ResourceLocation.parse("empty");
    ...
    public static final ResourceLocation WITHER_NECROMANCER = register("entities/wither_necromancer");
    ...
    public static LootParams.Builder createLootParams(LivingEntity target, boolean checkPlayerKill, DamageSource source) {
        LootParams.Builder lootcontext$builder = (new LootParams.Builder((ServerLevel) target.level())).withParameter(LootContextParams.THIS_ENTITY, target).withParameter(LootContextParams.ORIGIN, target.position()).withParameter(LootContextParams.DAMAGE_SOURCE, source).withOptionalParameter(LootContextParams.ATTACKING_ENTITY, source.getEntity()).withOptionalParameter(LootContextParams.DIRECT_ATTACKING_ENTITY, source.getDirectEntity());
        Player player = null;
        if (target.getKillCredit() instanceof Player killCredit) {
            player = killCredit;
        } else if (source.getEntity() instanceof Player attackingPlayer) {
            player = attackingPlayer;
        } else if (target.getLastHurtByMob() instanceof Player lastHurtByPlayer) {
    ...
    public static void createLootChest(LivingEntity target, BlockState blockState, BlockPos blockPos, DamageSource cause){
        createLootChest(target, blockState, blockPos, cause, target.getLootTable());
    }

    public static void createLootChest(LivingEntity target, BlockState blockState, BlockPos blockPos, DamageSource cause, ResourceKey<LootTable> lootTableKey){
