...
public class ServantUtil {
    ...
    public static void convertZombies(Entity target, LivingEntity owner, boolean permanent){
        if (target instanceof Zombie zombieEntity && net.neoforged.neoforge.event.EventHooks.canLivingConvert(zombieEntity, ModEntityType.ZOMBIE_SERVANT.get(), (timer) -> {})) {
            ...
            if (zombieServant != null) {
                ...
                if (target.level() instanceof ServerLevel serverLevel) {
                    zombieServant.finalizeSpawn(serverLevel, target.level().getCurrentDifficultyAt(zombieServant.blockPosition()), MobSpawnType.CONVERSION, null);
                }
                if (zombieEntity instanceof ZombieVillager villager && zombieServant instanceof ZombieVillagerServant servant){
                    ...
                    readOffers(villager, villagerData).ifPresent(offers -> servant.setTradeOffers(offers.copy()));
                    servant.setVillagerXp(villager.getVillagerXp());
                }
                if (!permanent) {
                    zombieServant.setLimitedLife(10 * (15 + target.level().random.nextInt(45)));
                }
                net.neoforged.neoforge.event.EventHooks.onLivingConvert(zombieEntity, zombieServant);
    ...
    public static void convertSkeletons(Entity target, LivingEntity owner, boolean wither, boolean permanent){
        if (target instanceof AbstractSkeleton skeleton && net.neoforged.neoforge.event.EventHooks.canLivingConvert(skeleton, ModEntityType.SKELETON_SERVANT.get(), (timer) -> {})) {
            ...
            if (skeletonServant != null) {
                ...
                if (target.level() instanceof ServerLevel serverLevel) {
                    skeletonServant.finalizeSpawn(serverLevel, target.level().getCurrentDifficultyAt(skeletonServant.blockPosition()), MobSpawnType.CONVERSION, null);
                }
                if (!permanent) {
            ...
            if (net.neoforged.neoforge.event.EventHooks.canLivingConvert(target, entityType, (timer) -> {})) {
                ...
                if (summoned instanceof ZombieVillagerServant servant) {
                    if (target instanceof Villager villager) {
                        ...
                        servant.setTradeOffers(villager.getOffers().copy());
                        servant.setVillagerXp(villager.getVillagerXp());
                        ...
                        if (prisoner.getOffers() != null) {
                            readOffers(prisoner, prisoner.getOffers()).ifPresent(servant::setTradeOffers);
                        }
    ...
    public static Prisoner takePrisoner(AbstractVillager villager) {
        ...
        if (prisoner != null) {
            ...
            prisoner.setTradeOffers((CompoundTag)MerchantOffers.CODEC.encodeStart(villager.registryAccess().createSerializationContext(NbtOps.INSTANCE), villager.getOffers()).getOrThrow());
    ...
    public static boolean isFrostHeal(LivingEntity servant){
        ModMobType mobType = MobTypeHelper.getMobType(servant);
    ...
    public static boolean isWildHeal(LivingEntity servant){
        ModMobType mobType = MobTypeHelper.getMobType(servant);
        EntityType<?> entityType = servant.getType();
        return mobType == ModMobType.NATURAL || mobType == ModMobType.ARTHROPOD || entityType.is(ModTags.EntityTypes.WILD_HEAL);
    }
    ...
    public static boolean isNecroHeal(LivingEntity servant){
        ModMobType mobType = MobTypeHelper.getMobType(servant);
        EntityType<?> entityType = servant.getType();
        return mobType == ModMobType.UNDEAD || entityType.is(ModTags.EntityTypes.NECRO_HEAL);
    }

    public static boolean isAbyssHeal(LivingEntity servant){
        ModMobType mobType = MobTypeHelper.getMobType(servant);
        EntityType<?> entityType = servant.getType();
        return mobType == ModMobType.WATER || entityType.is(ModTags.EntityTypes.ABYSS_HEAL);
    }
