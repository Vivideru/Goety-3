...
public class CuriosFinder {
        ...
        if (player != null) {
            if (CuriosFinder.findRing(player).getItem() == ModItems.RING_OF_WANT.get()){
                if (CuriosFinder.findRing(player).isEnchanted()){
                    float wanting = EnchantmentHelper.getTagEnchantmentLevel(ModEnchantments.holder(player, ModEnchantments.WANTING), CuriosFinder.findRing(player));
                    return wanting > 0;
                }
            }
        }
    ...
    public static boolean validWildMob(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.NATURAL
                || MobTypeHelper.getMobType(livingEntity) == ModMobType.ARTHROPOD
    ...
    public static boolean validNetherMob(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.NETHER
    ...
    public static boolean validNecroUndead(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.UNDEAD || livingEntity.getType().is(ModTags.EntityTypes.NECRO_SET_NEUTRAL)) && livingEntity.getMaxHealth() <= ItemConfig.NecroSetUndeadNeutralHealth.get() && !(livingEntity instanceof IOwned && !(livingEntity instanceof Enemy));
    }
    ...
    public static boolean validNamelessUndead(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.UNDEAD || livingEntity.getType().is(ModTags.EntityTypes.NECRO_SET_NEUTRAL)) && livingEntity.getMaxHealth() <= ItemConfig.NamelessSetUndeadNeutralHealth.get() && !(livingEntity instanceof IOwned && !(livingEntity instanceof Enemy));
    }
    ...
    public static boolean validFrostMob(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.FROST
    ...
    public static boolean validAbyssMob(LivingEntity livingEntity){
        return (MobTypeHelper.getMobType(livingEntity) == ModMobType.WATER
        ...
        if (livingEntity instanceof Player player && CuriosFinder.hasCurio(player, ModItems.RING_OF_THE_DRAGON.get())) {
            ...
            if (player.level() instanceof ServerLevel serverLevel && !player.getCooldowns().isOnCooldown(ring.getItem())) {
                ...
                serverLevel.sendParticles(new CircleExplodeParticleOption(color.red(), color.green(), color.blue(), radius, 1),
                        target.x, target.y + 0.25F, target.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
                serverLevel.sendParticles(new SphereExplodeParticleOption(color.red(), color.green(), color.blue(), radius, 1),
                        target.x, target.y + 0.25F, target.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
                for (int i = 0; i < Mth.floor(radius); ++i) {
                    serverLevel.sendParticles(ModParticleTypes.BIG_CULT_SPELL.get(),
                            target.x + serverLevel.getRandom().nextGaussian() / 16.0F,
                            target.y + 1.0F + serverLevel.getRandom().nextGaussian() / 16.0F,
                            target.z + serverLevel.getRandom().nextGaussian() / 16.0F,
                            0, color.red(), color.green(), color.blue(), 1.0F);
                }
                new SpellExplosion(serverLevel, player, serverLevel.damageSources().explosion(player, player),
                        target.x, target.y, target.z, radius, 0.0F);
                serverLevel.playSound(null, target.x, target.y, target.z, ModSounds.VOID_BLAST.get(),
                        livingEntity.getSoundSource(), 1.3F, 1.0F);
