...
public class RaidAdditions {
    ...
    public static final List<CustomRaiderType> NEW_RAID_MEMBERS = new ArrayList<>();

    public static void addRaiders(){
        NEW_RAID_MEMBERS.clear();
    ...
    private static int[] toWaveArray(List<? extends Integer> list) {
        int[] waves = new int[list.size()];
        for (int i = 0; i < list.size(); i++) {
            waves[i] = Math.max(0, list.get(i));
        }
        return waves;
    }

    public record CustomRaiderType(String name, EntityType<? extends Raider> entityType, int[] spawnsPerWave) {
        public int getDefaultNumSpawns(int wave) {
            if (wave >= 0 && wave < this.spawnsPerWave.length) {
                return this.spawnsPerWave[wave];
            }
            return 0;
        }
    }
