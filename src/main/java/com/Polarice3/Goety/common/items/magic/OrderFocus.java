...
public class OrderFocus extends MagicFocus{
    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (!worldIn.isClientSide) {
            ...
            if (listTag != null && stack.has(DataComponents.CUSTOM_DATA)){
                if (listTag.isEmpty()){
                    updateTag(stack, compound -> {
                        compound.remove(SERVANT_LIST);
                        compound.remove(SERVANT_CLIENT_LIST);
                    });
                }
            }
    ...
    public boolean isFoil(ItemStack p_41453_) {
        return tag(p_41453_).contains(SERVANT_LIST);
    }
    ...
    public static void addCommandText(Level level, ItemStack stack, List<Component> tooltip){
        ...
        if (!compound.isEmpty()) {
            if (!compound.contains(SERVANT_LIST)) {
                ...
                if (level != null) {
                    for (LivingEntity livingEntity : getServantsClient(level, compound)) {
                        if (livingEntity != null) {
