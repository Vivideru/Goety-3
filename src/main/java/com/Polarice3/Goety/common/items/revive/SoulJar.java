...
public class SoulJar extends ReviveServantItem {
                ...
                if (necromancer.getTrueOwner() == player) {
                    if (RitualRequirements.canSummon(level, player, ModEntityType.NECROMANCER_SERVANT.get())) {
                        ...
                        if (level.addFreshEntity(necromancer)) {
                            ...
                            necromancer.playSound(SoundEvents.GENERIC_EXPLODE.value(), 1.0F, 0.5F);
    ...
    public static boolean isCairn(ItemStack stack) {
        ...
        return stack.getItem() instanceof SoulJar && !compoundtag.isEmpty() && compoundtag.contains(TAG_CAIRN);
    }

    public static void setCairn(ItemStack stack){
        updateTag(stack, tag -> tag.putBoolean(TAG_CAIRN, true));
    }

    public static boolean isMossy(ItemStack stack) {
        CompoundTag compoundtag = tag(stack);
        return stack.getItem() instanceof SoulJar && !compoundtag.isEmpty() && compoundtag.contains(TAG_MOSSY);
    }

    public static void setMossy(ItemStack stack){
        updateTag(stack, tag -> tag.putBoolean(TAG_MOSSY, true));
    }

    public static boolean isDrowned(ItemStack stack) {
        CompoundTag compoundtag = tag(stack);
        return stack.getItem() instanceof SoulJar && !compoundtag.isEmpty() && compoundtag.contains(TAG_DROWNED);
    }

    public static void setDrowned(ItemStack stack){
        updateTag(stack, tag -> tag.putBoolean(TAG_DROWNED, true));
    }

    public static boolean isWither(ItemStack stack) {
        CompoundTag compoundtag = tag(stack);
        return stack.getItem() instanceof SoulJar && !compoundtag.isEmpty() && compoundtag.contains(TAG_WITHER);
    }

    public static void setWither(ItemStack stack){
        updateTag(stack, tag -> tag.putBoolean(TAG_WITHER, true));
    }
    ...
    public static void setNecromancer(AbstractNecromancer necromancer, ItemStack stack) {
        ...
        ResourceLocation typesKey = BuiltInRegistries.ENTITY_TYPE.getKey(necromancer.getType());
