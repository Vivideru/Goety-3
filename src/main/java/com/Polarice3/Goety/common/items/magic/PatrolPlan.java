...
/**
 * Charts a cycle of waypoints that owned servants patrol once the plan is used on them (or blown to them with a
 * Command Horn held in the other hand). Based on @Minecraft-LightLand's ModularGolems PathRecordCard.
 */
public class PatrolPlan extends Item {
    ...
    private static final double MAX_WAYPOINT_DISTANCE_SQR = 256.0D;

    public PatrolPlan(Properties properties) {
        super(properties.rarity(Rarity.UNCOMMON));
    }

    public static List<GlobalPos> getList(ItemStack stack) {
        CompoundTag tag = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        if (!tag.contains(PATROL_PATH, Tag.TAG_LIST)) {
    ...
    public static void setList(ItemStack stack, List<GlobalPos> list) {
        ...
        if (list.isEmpty()) {
            ...
            LIST_CODEC.encodeStart(NbtOps.INSTANCE, list)
                    .resultOrPartial(Goety.LOGGER::error)
                    .ifPresent(encoded -> tag.put(PATROL_PATH, encoded));
        }
    ...
    public InteractionResult useOn(UseOnContext ctx) {
        ...
        if (!level.isClientSide()) {
            ...
            if (!list.isEmpty()) {
                ...
                if (!list.contains(entry) && entry.pos().distSqr(last.pos()) > MAX_WAYPOINT_DISTANCE_SQR) {
                    if (player != null) {
            ...
            boolean added = togglePos(stack, entry);
            if (player != null) {
                player.displayClientMessage(Component.translatable(added ? "info.goety.patrol.add" : "info.goety.patrol.remove"), true);
                ModNetwork.sendTo(player, new SPlayPlayerSoundPacket(SoundEvents.BOOK_PAGE_TURN, 1.0F, added ? 1.0F : 0.5F));
            }
        }
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        if (!player.level().isClientSide
                && target instanceof IServant servant
                && servant.canPatrol()
    ...
    public static void assignRoute(IServant servant, List<GlobalPos> route) {
        servant.setPatrolRoute(new ArrayList<>(route));
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("info.goety.patrol.number", getList(stack).size()).withStyle(ChatFormatting.BLUE));
        tooltip.add(Component.translatable("info.goety.patrol").withStyle(ChatFormatting.DARK_PURPLE));
    }
}
