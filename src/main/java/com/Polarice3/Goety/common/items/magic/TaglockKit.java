...
public class TaglockKit extends Item {
    ...
    public void inventoryTick(@NotNull ItemStack stack, Level worldIn, @NotNull Entity entityIn, int itemSlot, boolean isSelected) {
        if (!worldIn.isClientSide) {
            if (stack.has(DataComponents.CUSTOM_DATA)){
                ...
                LivingEntity livingEntity = getEntity(compound);
                if (livingEntity != null && livingEntity.isDeadOrDying()){
                    updateTag(stack, data -> {
                        data.remove(TAG_ENTITY);
                        data.remove(TAG_PLAYER_NAME);
                    });
                } else if (livingEntity == null && !compound.contains(TAG_PLAYER_NAME)) {
                    updateTag(stack, data -> data.remove(TAG_ENTITY));
                }
    ...
    public static long getTicketTime(ItemStack itemStack) {
        if (itemStack.has(DataComponents.CUSTOM_DATA)) {
            return tag(itemStack).getInt(TAG_TICKET);
        } else {
            return 0;
        }
    }
    ...
    public static void setTicketTime(ItemStack itemStack, long time) {
        if (itemStack.has(DataComponents.CUSTOM_DATA)) {
            updateTag(itemStack, tag -> tag.putInt(TAG_TICKET, (int) time));
        }
    }
        ...
        if (!compound.isEmpty()) {
            if (hasEntity(stack)) {
                ...
                if (livingEntity != null){
                    ...
                    tooltip.add(Component.translatable("info.goety.taglock.tagged").append(" ")
                            .append(compound.getString(TAG_PLAYER_NAME))
                            .withStyle(ChatFormatting.GRAY));
                }
            } else if (hasPlayerName(stack)) {
                tooltip.add(Component.translatable("info.goety.taglock.tagged").append(" ")
