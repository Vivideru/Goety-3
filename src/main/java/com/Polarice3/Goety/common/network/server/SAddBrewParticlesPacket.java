...
public class SAddBrewParticlesPacket {
    ...
    public static SAddBrewParticlesPacket decode(FriendlyByteBuf buffer) {
        return new SAddBrewParticlesPacket(ItemStack.OPTIONAL_STREAM_CODEC.decode((RegistryFriendlyByteBuf) buffer), buffer.readBlockPos(), buffer.readBoolean(), buffer.readInt());
    }

    public static void consume(SAddBrewParticlesPacket packet, Supplier<ModNetwork.Context> ctx) {
        ctx.get().enqueueWork(() -> {
