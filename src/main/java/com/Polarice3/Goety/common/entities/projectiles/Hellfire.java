...
public class Hellfire extends GroundProjectile {
    ...
    private static final int HURT_COOLDOWN = 10;
    public boolean isDying;
    private final Map<Integer, Integer> hurtCooldowns = new HashMap<>();
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            ...
            this.level().addParticle(new MagicSmokeParticleOption(0xcf7a06, 0xb13f00, 10 + this.level().getRandom().nextInt(10), 0.2F), this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0.0D, 0.0D, 0.0D);
    ...
    private void tickHurtCooldowns() {
        Iterator<Map.Entry<Integer, Integer>> iterator = this.hurtCooldowns.entrySet().iterator();
    ...
    public void dealDamageTo(LivingEntity target) {
        ...
        if (target.isAlive() && !target.isInvulnerable()) {
            ...
            boolean hurt;
            if (owner == null) {
                hurt = target.hurt(ModDamageSource.getDamageSource(this.level(), ModDamageSource.HELLFIRE), damage);
            } else {
                if (target == owner){
                    return;
                }
                ...
                hurt = target.hurt(ModDamageSource.hellfire(this, this.getOwner()), damage);
            }
            if (hurt) {
                this.hurtCooldowns.put(target.getId(), HURT_COOLDOWN);
            }
        }
    }
