...
public class AbstractMossyNecromancer extends AbstractNecromancer{
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.MossyNecromancerHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.MossyNecromancerArmor))
                .add(Attributes.FOLLOW_RANGE, AttributesConfig.get(AttributesConfig.MossyNecromancerFollowRange))
                .add(Attributes.MOVEMENT_SPEED, 0.25F)
                .add(Attributes.KNOCKBACK_RESISTANCE, 1.0D)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.MossyNecromancerDamage));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.MossyNecromancerHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.MossyNecromancerArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.FOLLOW_RANGE), AttributesConfig.get(AttributesConfig.MossyNecromancerFollowRange));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.MossyNecromancerDamage));
    }
    ...
    public class SummonServantSpell extends SummoningSpellGoal {
        ...
        public boolean canUse() {
            return super.canUse() && AbstractMossyNecromancer.this.remainingOwnedSummonSlots(6) > 0;
        }

        protected void castSpell(){
            if (AbstractMossyNecromancer.this.level() instanceof ServerLevel serverLevel) {
                int slots = AbstractMossyNecromancer.this.remainingOwnedSummonSlots(6);
                for (int i1 = 0; i1 < Math.min(2, slots); ++i1) {
                    ...
                    summonedentity.setTrueOwner(MobUtil.getSummonOwner(AbstractMossyNecromancer.this));
