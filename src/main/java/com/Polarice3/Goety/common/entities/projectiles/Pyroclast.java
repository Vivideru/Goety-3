    ...
    public void explode(){
        if (!this.level().isClientSide) {
            ...
            if (owner != null && owner.getType().getDescriptionId().contains("netherite_monstrosity")) {
                ...
                float damage = SpellConfig.get(SpellConfig.PyroclastDamage).floatValue() * WandUtil.damageMultiply();
                    ...
                    public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z, double seen, float actualDamage) {
                        ...
                        if (Pyroclast.this.isUpgraded()) {
                            if (target instanceof LivingEntity target1 && !MobUtil.areAllies(owner, target1) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(target1)) {
                                target1.addEffect(new MobEffectInstance(GoetyEffects.STUNNED, MathHelper.secondsToTicks(2), 0, false, false));
                            }
                        }
                    }
                };
            ...
            if (this.isUpgraded()) {
                if (!(entity1 != null && entity1.getType().getDescriptionId().contains("netherite_monstrosity"))) {
                    if (entity instanceof LivingEntity livingEntity) {
                        livingEntity.addEffect(new MobEffectInstance(GoetyEffects.STUNNED, MathHelper.secondsToTicks(2), 0, false, false));
                    }
                }
            }
            if (this.getFlaming() != 0){
                entity.igniteForSeconds(5.0F * this.getFlaming());
            }
        }
        if (entity1 instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                EnchantmentHelper.doPostAttackEffects(serverLevel, entity, ModDamageSource.modFireball(this.getOwner(), this.level()));
            }
        }
    }
