...
public class Rattled extends AbstractSkeleton implements ICustomAttributes {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.RattledServantHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.RattledServantArmor))
                .add(Attributes.MOVEMENT_SPEED, 0.25F)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.RattledServantDamage));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.RattledServantHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.RattledServantArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.RattledServantDamage));
    }
    ...
    public double getBaseRangeDamage(){
        return AttributesConfig.get(AttributesConfig.RattledServantRangeDamage);
    }
    ...
    protected AbstractArrow getArrow(ItemStack pArrowStack, float pDistanceFactor, ItemStack pWeapon) {
        AbstractArrow abstractarrowentity = super.getArrow(pArrowStack, pDistanceFactor, pWeapon);
        ...
        if (abstractarrowentity instanceof Arrow) {
            ((Arrow)abstractarrowentity).addEffect(new MobEffectInstance(GoetyEffects.SPASMS, MathHelper.secondsToTicks(3), 0));
        }
    ...
    public boolean canBeAffected(MobEffectInstance pPotioneffect) {
        return super.canBeAffected(pPotioneffect)
                && !pPotioneffect.getEffect().is(GoetyEffects.SPASMS);
    }
