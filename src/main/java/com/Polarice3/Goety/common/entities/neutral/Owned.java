...
public class Owned extends PathfinderMob implements IOwned, OwnableEntity, ICustomAttributes {
    ...
    public void maybeDisableShield(Player player, ItemStack axe, ItemStack shield) {
        if (!axe.isEmpty() && !shield.isEmpty() && axe.getItem() instanceof AxeItem && shield.is(Items.SHIELD)) {
            float f = 0.25F + (float)this.getMainHandItem().getEnchantmentLevel(this.level().registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(net.minecraft.world.item.enchantment.Enchantments.EFFICIENCY)) * 0.05F;
    ...
    protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
        ...
        pBuilder.define(OWNER_CLIENT_ID, -1);
        pBuilder.define(HOSTILE, false);
        pBuilder.define(NATURAL, false);
    }
    ...
    public boolean canBeAffected(MobEffectInstance pPotioneffect) {
        if (this.getTrueOwner() != null) {
            return !pPotioneffect.getEffect().is(GoetyEffects.GOLD_TOUCHED) && super.canBeAffected(pPotioneffect);
        }
    ...
    public static boolean checkNetherAwareHostileSpawnRules(EntityType<? extends Owned> pType, ServerLevelAccessor pLevel, MobSpawnType pReason, BlockPos pPos, RandomSource pRandom) {
        ...
        return checkHostileSpawnRules(pType, pLevel, pReason, pPos, pRandom);
    }
    ...
    public static boolean checkNetherAwareDayMonsterSpawnRules(EntityType<? extends Owned> pType, ServerLevelAccessor pLevel, MobSpawnType pReason, BlockPos pPos, RandomSource pRandom) {
        ...
        return checkDayMonsterSpawnRules(pType, pLevel, pReason, pPos, pRandom);
    }
