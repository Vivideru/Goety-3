    ...
    public void spellParticles(){
        if (this.level().isClientSide && this.isCastingSpell()) {
            ...
            if (spell != IllagerServantSpell.NONE) {
                ...
                float f1 = Mth.cos(f);
                float f2 = Mth.sin(f);
                ColorParticleOption particle = ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, (float)d0, (float)d1, (float)d2);
                this.level().addParticle(particle, this.getX() + (double) f1 * 0.6D, this.getY() + 1.8D, this.getZ() + (double) f2 * 0.6D, 0.0D, 0.0D, 0.0D);
                this.level().addParticle(particle, this.getX() - (double) f1 * 0.6D, this.getY() + 1.8D, this.getZ() - (double) f2 * 0.6D, 0.0D, 0.0D, 0.0D);
            }
        }
    }
