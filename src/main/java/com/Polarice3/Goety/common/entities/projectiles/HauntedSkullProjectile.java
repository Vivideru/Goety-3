...
public class HauntedSkullProjectile extends ExplosiveProjectile{
    ...
    public float damage = SpellConfig.get(SpellConfig.HauntedSkullDamage).floatValue() * WandUtil.damageMultiply();
            ...
            if (owner instanceof LivingEntity livingentity) {
                ...
                if (flag) {
                    if (target.isAlive()) {
                        ...
                        if (flaming != 0) {
                            target.igniteForSeconds(5.0F * flaming);
                        }
                    } else {
    ...
    public void explode(){
        ...
        if (!this.level().isClientSide) {
            ...
            if (owner instanceof Player player) {
                if (CuriosFinder.findRing(player).getItem() == ModItems.RING_OF_WANT.get()) {
                    if (CuriosFinder.findRing(player).isEnchanted()) {
                        float wanting = EnchantmentHelper.getTagEnchantmentLevel(ModEnchantments.holder(this, ModEnchantments.WANTING), CuriosFinder.findRing(player));
                        if (wanting > 0) {
                            loot = true;
                        }
                    }
            ...
            if (this.getOwner() instanceof Player) {
                damaging = SpellConfig.get(SpellConfig.HauntedSkullGriefing);
            } else {
                damaging = net.neoforged.neoforge.event.EventHooks.canEntityGrief(this.level(), this);
            }
            if (!damaging) {
                flaming = false;
            }
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (this.isUpgraded() || this.exploding) {
            return false;
        } else {
            this.explode();
            this.markHurt();
