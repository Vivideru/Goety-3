...
public class Piker extends HuntingIllagerEntity{
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.PikerHealth))
                .add(Attributes.FOLLOW_RANGE, 32.0D)
                .add(Attributes.MOVEMENT_SPEED, 0.35D)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.PikerDamage))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.PikerArmor));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.PikerHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.PikerDamage));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.PikerArmor));
    }
    ...
    @Nullable
    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor accessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData groupData) {
        ...
        if (spawnType == MobSpawnType.EVENT) {
            if (accessor.getLevel().random.nextFloat() <= 0.25F && !this.isPassenger()) {
                ...
                trampler.finalizeSpawn(accessor, accessor.getLevel().getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.EVENT, null);
    ...
    public boolean doHurtTarget(Entity p_21372_) {
        ...
        DamageSource damageSource = p_21372_.damageSources().mobAttack(this);
        if (p_21372_ instanceof LivingEntity) {
            f = MobUtil.getEnchantedDamage(this, this.getMainHandItem(), p_21372_, damageSource, f);
            f1 = MobUtil.getEnchantedKnockback(this, this.getMainHandItem(), p_21372_, damageSource, f1);
        }
    ...
    class PikerAttackGoal extends MeleeAttackGoal {
        ...
        protected void checkAndPerformAttack(LivingEntity enemy) {
            ...
            double distToEnemySqr = Piker.this.distanceToSqr(enemy.getX(), enemy.getBoundingBox().minY, enemy.getZ());
