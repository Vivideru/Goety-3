...
public class Spike extends GroundProjectile {
    ...
    private void dealDamageTo(LivingEntity target) {
        ...
        if (target.isAlive() && !target.isInvulnerable() && target != livingEntity) {
            boolean flag;
            if (livingEntity == null) {
                flag = target.hurt(ModDamageSource.spike(this, this), SpellConfig.get(SpellConfig.SpikeDamage).floatValue() + this.getExtraDamage());
            } else {
                if (MobUtil.areAllies(livingEntity, target)){
                    return;
                }
                flag = target.hurt(ModDamageSource.spike(this, livingEntity), SpellConfig.get(SpellConfig.SpikeDamage).floatValue() + this.getExtraDamage());
            }
            if (flag){
                if (livingEntity instanceof Player player) {
                    int soulEater = Mth.clamp(this.getSoulEater(), 0, 10);
                    SEHelper.increaseSouls(player, SpellConfig.get(SpellConfig.SpikeGainSouls) * soulEater);
                    if (this.getBurning() > 0){
                        target.igniteForSeconds(5.0F * this.getBurning());
                    }
                }
            }
        }
