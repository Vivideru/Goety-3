...
public abstract class AbstractBoundIllager extends RaiderServant implements IMobTyped {
    ...
    public int getSummonLimit(LivingEntity owner) {
        return SpellConfig.get(SpellConfig.BoundIllagerLimit);
    }
    ...
    public void tick() {
        ...
        if (this.level().isClientSide && this.isCastingSpell() && this.getCurrentSpell() != BoundSpell.CLOUDLESS) {
            ...
            float f1 = Mth.cos(f);
            float f2 = Mth.sin(f);
            this.level().addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, (float)d0, (float)d1, (float)d2), this.getX() + (double)f1 * 0.6D, this.getY() + 1.8D, this.getZ() + (double)f2 * 0.6D, 0.0D, 0.0D, 0.0D);
            this.level().addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, (float)d0, (float)d1, (float)d2), this.getX() - (double)f1 * 0.6D, this.getY() + 1.8D, this.getZ() - (double)f2 * 0.6D, 0.0D, 0.0D, 0.0D);
        }
