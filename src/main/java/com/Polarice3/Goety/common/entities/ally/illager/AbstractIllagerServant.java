...
public abstract class AbstractIllagerServant extends RaiderServant implements ITrainable, ILooter, IMobTyped, ICharmUser {
                        ...
                        if (!itemstack.isEmpty()) {
                            ...
                            if (integer != null) {
                                ...
                                if (foodProperties != null) {
                                    if (!foodProperties.effects().isEmpty()) {
                                        for(FoodProperties.PossibleEffect effect : foodProperties.effects()) {
                                            if (this.getRandom().nextFloat() < effect.probability()) {
                                ...
                                if (!BrewUtils.isEmpty(itemstack)){
                                    if (!ModPotionUtil.getMobEffects(itemstack).isEmpty()) {
                                        for (MobEffectInstance instance : ModPotionUtil.getMobEffects(itemstack)) {
                                            if (instance.getEffect().value().isInstantenous()) {
                                                instance.getEffect().value().applyInstantenousEffect(this, this, this, instance.getAmplifier(), 1.0D);
                                            } else {
    ...
    public AbstractIllagerServant getBreedOffspring(ServerLevel serverLevel, AbstractIllagerServant illager) {
        ...
        EventHooks.finalizeMobSpawn(neollager, serverLevel, serverLevel.getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.BREEDING, null);
    ...
    public int trainSpeed(EntityType<? extends Mob> entityType) {
        ...
        if (this.hasEffect(GoetyEffects.INSIGHT)) {
            MobEffectInstance instance = this.getEffect(GoetyEffects.INSIGHT);
            if (instance != null) {
    ...
    protected void dropAllInventory() {
        for(int i = 0; i < this.getInventory().getContainerSize(); ++i) {
            ItemStack itemstack = this.getInventory().getItem(i);
            if (!itemstack.isEmpty()) {
                if (EnchantmentHelper.getTagEnchantmentLevel(this.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.VANISHING_CURSE), itemstack) > 0) {
    ...
    public static class GiveExcessFoodGoal extends ThrowItemGoal {
        ...
        public GiveExcessFoodGoal(AbstractIllagerServant illager){
            super(illager);
            this.predicate = itemStack -> itemStack.getFoodProperties(this.illager) != null;
        ...
        public void throwItem() {
            ...
            for (int i = 0; i < simpleContainer.getContainerSize(); ++i){
                ...
                if (!itemstack1.isEmpty()) {
                    if (itemstack1.getFoodProperties(this.illager) != null) {
                        int j = 0;
