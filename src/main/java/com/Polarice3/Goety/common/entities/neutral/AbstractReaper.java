...
public class AbstractReaper extends Summoned implements IMobTyped {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.ReaperHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.ReaperArmor))
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.ReaperDamage))
    ...
    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.ReaperHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.ReaperArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.ReaperDamage));
    }
    ...
    public int getSummonLimit(LivingEntity owner) {
        return SpellConfig.get(SpellConfig.ReaperLimit);
    }
    ...
    static class MeleeGoal extends Goal {
        ...
        public void attackMobs(LivingEntity pTarget, AbstractReaper reaper){
            ...
            DamageSource damageSource = this.reaper.getServantAttack();
            float f1 = MobUtil.getEnchantedDamage(reaper, reaper.getMainHandItem(), pTarget, damageSource, f) - f;
            float f2 = 1.0F;
            f = f * (0.2F + f2 * f2 * 0.8F);
            f1 = f1 * f2;
            f = f + f1;

            float f3 = 1.0F + MobUtil.getSweepingDamageRatio(3) * f;
            int j = MobUtil.getFireAspect(reaper);
            double area = 2.0D;
            for (LivingEntity livingentity : reaper.level().getEntitiesOfClass(LivingEntity.class, this.reaper.getBoundingBox().move(this.getHorizontalLookAngle().scale(2.0D)).inflate(area, area, area))) {
                if (livingentity != reaper && !MobUtil.areAllies(reaper, livingentity) && (!(livingentity instanceof ArmorStand) || !((ArmorStand) livingentity).isMarker()) && reaper.distanceToSqr(livingentity) < 16.0D && livingentity != reaper.getVehicle()) {
                    ...
                    if (livingentity.hurt(damageSource, f3)) {
                        ...
                        MobUtil.doPostAttackEffects(reaper, livingentity, damageSource, reaper.getMainHandItem());
                    }
                }
            }
