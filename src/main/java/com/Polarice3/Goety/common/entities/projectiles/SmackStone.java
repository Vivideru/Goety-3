...
public class SmackStone extends SpellThrowableProjectile {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(DATA_DAMAGE, SpellConfig.get(SpellConfig.SmackStoneDamage).floatValue() * WandUtil.damageMultiply());
    ...
    protected void onHitEntity(EntityHitResult p_37404_) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (owner instanceof LivingEntity && this.level() instanceof ServerLevel serverLevel) {
                if (flag) {
                    if (target.isAlive()) {
                        EnchantmentHelper.doPostAttackEffects(serverLevel, target, target.damageSources().thrown(this, this.getOwner()));
                    }
                }
            }

            if (flag && target instanceof LivingEntity livingEntity) {
                livingEntity.addEffect(new MobEffectInstance(GoetyEffects.PLUNGE, MathHelper.secondsToTicks(5)));
                if (this.getFiery() > 0) {
                    livingEntity.igniteForSeconds(5 * (this.getFiery() + 1));
                }
            }
