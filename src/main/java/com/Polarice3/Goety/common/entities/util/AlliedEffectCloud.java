...
public class AlliedEffectCloud extends Entity implements TraceableEntity {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final int TIME_BETWEEN_APPLICATIONS = 5;
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(AlliedEffectCloud.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> DATA_COLOR = SynchedEntityData.defineId(AlliedEffectCloud.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> DATA_WAITING = SynchedEntityData.defineId(AlliedEffectCloud.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<ParticleOptions> DATA_PARTICLE = SynchedEntityData.defineId(AlliedEffectCloud.class, EntityDataSerializers.PARTICLE);
    ...
    public AlliedEffectCloud(EntityType<? extends AlliedEffectCloud> p_19704_, Level p_19705_) {
        super(p_19704_, p_19705_);
        this.noPhysics = true;
    }

    public AlliedEffectCloud(Level p_19707_, double p_19708_, double p_19709_, double p_19710_) {
        this(ModEntityType.ALLIED_EFFECT_CLOUD.get(), p_19707_);
    ...
    protected Component getTypeName() {
        return EntityType.AREA_EFFECT_CLOUD.getDescription();
    }
    ...
    public void setPotion(Holder<Potion> p_19723_) {
    ...
    private void updateColor() {
        if (this.potion == Potions.WATER && this.effects.isEmpty()) {
            ...
            this.getEntityData().set(DATA_COLOR, ModPotionUtil.getColor(ModPotionUtil.getAllEffects(this.potion, this.effects)));
        }
                ...
                if (list.isEmpty()) {
                    ...
                    if (!list1.isEmpty()) {
                        for(LivingEntity livingentity : list1) {
                            if (this.getOwner() == null || !MobUtil.areAllies(this.getOwner(), livingentity)) {
    ...
    protected void readAdditionalSaveData(CompoundTag p_19727_) {
        ...
        if (p_19727_.contains("Potion", 8)) {
            this.setPotion(BuiltInRegistries.POTION.getHolder(ResourceLocation.parse(p_19727_.getString("Potion"))).<Holder<Potion>>map(holder -> holder).orElse(Potions.WATER));
        }
    ...
    protected void addAdditionalSaveData(CompoundTag p_19737_) {
        ...
        if (this.potion != Potions.WATER) {
            p_19737_.putString("Potion", BuiltInRegistries.POTION.getKey(this.potion.value()).toString());
        }
