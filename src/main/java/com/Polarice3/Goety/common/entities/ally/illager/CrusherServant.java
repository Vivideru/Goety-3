...
public class CrusherServant extends AbstractIllagerServant implements IMobCrafter {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.CrusherHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.CrusherArmor))
                .add(Attributes.FOLLOW_RANGE, 16.0D)
                .add(Attributes.MOVEMENT_SPEED, 0.35D)
                .add(Attributes.KNOCKBACK_RESISTANCE, 0.75D)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.CrusherDamage));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.CrusherHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.CrusherArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.CrusherDamage));
    }
    ...
    private static List<CraftingRecipe> getArmorRecipes(ServerLevel level) {
        if (ARMOR_RECIPE_CACHE == null) {
            ARMOR_RECIPE_CACHE = level.getRecipeManager()
                    ...
                    .filter(r -> validCraft(r.value().getResultItem(level.registryAccess()))
                            && r.value().getResultItem(level.registryAccess()).getItem() instanceof ArmorItem)
    ...
    private static List<CraftingRecipe> getWeaponRecipes(ServerLevel level) {
        if (WEAPON_RECIPE_CACHE == null) {
            WEAPON_RECIPE_CACHE = level.getRecipeManager()
                    .getAllRecipesFor(RecipeType.CRAFTING).stream()
                    .filter(r -> validWeapon(r.value().getResultItem(level.registryAccess())))
                    .map(RecipeHolder::value)
                    .toList();
        }
    ...
    public static boolean validWeapon(ItemStack itemStack) {
        ...
        return itemStack.getItem() instanceof TieredItem
                ...
                || itemStack.is(Tags.Items.TOOLS_CROSSBOW)
    ...
    class AttackGoal extends MeleeAttackGoal {
        ...
        protected void checkAndPerformAttack(@NotNull LivingEntity enemy) {
            double distToEnemySqr = CrusherServant.this.distanceToSqr(enemy.getX(), enemy.getY(), enemy.getZ());
        ...
        public void tick() {
            ...
            if (target != this.crusher && !this.crusher.isWithinDistance(target, 2.0D)) {
                ...
                for (EquipmentSlot slot : armorSlots()) {
                    ...
                    this.crusher.level().playSound(null, target.blockPosition(), SoundEvents.ARMOR_EQUIP_GENERIC.value(), crusher.getSoundSource(), 1.0F, 1.0F);
                }
            }
        }
    }
        ...
        public void tick() {
            ...
            if (!this.crusher.isWithinDistance(ally, 2.0D)) {
                ...
                if (weapon != null) {
                    ...
                    this.crusher.getInventory().setChanged();
                    this.crusher.level().playSound(null, ally.blockPosition(), SoundEvents.ARMOR_EQUIP_GENERIC.value(), this.crusher.getSoundSource(), 1.0F, 1.0F);
                }
            }
        }
    }
