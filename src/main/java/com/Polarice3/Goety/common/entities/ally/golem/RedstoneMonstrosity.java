...
public class RedstoneMonstrosity extends RaiderGolemServant implements PlayerRideable, IAutoRideable, IRM {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.RedstoneMonstrosityHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.RedstoneMonstrosityArmor))
                .add(Attributes.MOVEMENT_SPEED, 0.23D)
                .add(Attributes.KNOCKBACK_RESISTANCE, 1.0D)
                .add(Attributes.ATTACK_KNOCKBACK, 6.0D)
                .add(Attributes.STEP_HEIGHT, 2.0D)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.RedstoneMonstrosityDamage))
                .add(Attributes.FOLLOW_RANGE, AttributesConfig.get(AttributesConfig.RedstoneMonstrosityFollowRange));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.RedstoneMonstrosityHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.RedstoneMonstrosityArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.RedstoneMonstrosityDamage));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.FOLLOW_RANGE), AttributesConfig.get(AttributesConfig.RedstoneMonstrosityFollowRange));
    }
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (this.canHurtRange(pSource) > AttributesConfig.get(AttributesConfig.RedstoneMonstrosityHurtRange)
    ...
    protected void actuallyHurt(DamageSource source, float amount) {
        if (!source.is(DamageTypeTags.BYPASSES_INVULNERABILITY)){
            amount = Math.min(amount, AttributesConfig.get(AttributesConfig.RedstoneMonstrosityDamageCap).floatValue());
        }
                        ...
                        if (this.summonTick == MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 1)){
                            ...
                            for (LivingEntity target : this.level().getEntitiesOfClass(LivingEntity.class, aabb.inflate(MELEE_RANGE / 2.0F))) {
                                if (target != this && !MobUtil.areAllies(this, target)) {
                                    if (target.hurt(this.getServantAttack(), MobUtil.getSpecialAttackDamage(this, 2.0F))){
                                        float f1 = 2.0F;
    ...
    static class MeleeGoal extends Goal {
        ...
        public void hurtTarget(Entity target) {
            ...
            if (target instanceof LivingEntity livingEntity){
                f += (livingEntity.getMaxHealth() * AttributesConfig.get(AttributesConfig.RedstoneMonstrosityHPPercentDamage).floatValue());
            }
            ...
            if (flag) {
                ...
                if (this.mob.level() instanceof ServerLevel serverLevel) {
                    EnchantmentHelper.doPostAttackEffects(serverLevel, target, this.mob.getServantAttack());
                }
    ...
    public static class SlowRotMoveControl extends MobUtil.noSpinControl {
        ...
        private boolean isWalkable(float p_24997_, float p_24998_) {
            ...
            if (pathnavigation != null) {
                ...
                return nodeevaluator == null || WalkNodeEvaluator.getPathTypeStatic(this.mob, BlockPos.containing(this.mob.getX() + (double) p_24997_,
                        this.mob.getBlockY(), this.mob.getZ() + (double) p_24998_)) == PathType.WALKABLE;
            }
            return true;
        }
    }
