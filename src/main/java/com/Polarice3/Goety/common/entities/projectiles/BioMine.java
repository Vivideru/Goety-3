...
public class BioMine extends SpellEntity {
    ...
    public void trigger(){
        if (!this.level().isClientSide) {
            double bbSize = 3.0D + this.getExtraRadius();
            float damage = SpellConfig.get(SpellConfig.BiomineAcidDamage).floatValue() * WandUtil.damageMultiply();
            ...
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                serverLevel.sendParticles(new SphereExplodeParticleOption(colorUtil.red, colorUtil.green, colorUtil.blue, (float) bbSize, 1), this.getX(), this.getY(), this.getZ(), 1, 0.0D, 0.0D, 0.0D, 0.0D);
    ...
    public void explodeDamage(LivingEntity livingEntity) {
        if (!this.level().isClientSide) {
            float damage = SpellConfig.get(SpellConfig.BiomineDamage).floatValue() * WandUtil.damageMultiply();
            damage += this.getExtraDamage();
            if (this.getOwner() != null){
