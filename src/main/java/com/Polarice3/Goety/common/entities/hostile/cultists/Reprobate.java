...
public class Reprobate extends Cultist {
    private static final EntityDataAccessor<Integer> ANIM_STATE = SynchedEntityData.defineId(Reprobate.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> BARREL = SynchedEntityData.defineId(Reprobate.class, EntityDataSerializers.BOOLEAN);
    ...
    public Reprobate(EntityType<? extends Cultist> type, Level worldIn) {
        super(type, worldIn);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(1, new WitchBarterGoal(this){
            ...
            public void start() {
                super.start();
                Reprobate.this.setAnimationState(INSPECT);
            }

            @Override
            public void stop() {
                super.stop();
                Reprobate.this.setAnimationState(IDLE);
            }
        });
        ...
        this.goalSelector.addGoal(1, new AvoidTargetGoal<>(this, LivingEntity.class, 12, 1.2D, 1.2D){
            ...
            public boolean canUse() {
                return Reprobate.this.fleeTime > 0 && super.canUse();
            }
        });
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.ReprobateHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.ReprobateArmor))
                .add(Attributes.MOVEMENT_SPEED, 0.25D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.ReprobateHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.ReprobateArmor));
    }

    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        super.defineSynchedData(builder);
        builder.define(ANIM_STATE, 0);
        builder.define(BARREL, false);
    }
    ...
    public static class ThrowGoal extends Goal {
        public Reprobate reprobate;
        public int throwTick;

        public ThrowGoal(Reprobate reprobate) {
    ...
    public static class InciteGoal extends Goal {
        ...
        public InciteGoal(Reprobate reprobate) {
            this.reprobate = reprobate;
            this.setFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK));
        }
        ...
        public boolean canUse() {
            if (this.reprobate.inciteCool <= 0) {
                if (this.reprobate.hasActiveRaid()) {
        ...
        public List<Raider> getList() {
            List<Raider> list = this.reprobate.level().getEntitiesOfClass(Raider.class, this.reprobate.getBoundingBox().inflate(8.0D));
            list.removeIf(raider -> raider.getType() == this.reprobate.getType());
            list.removeIf(raider -> !raider.hasActiveRaid());
        ...
        public void tick() {
            ...
            if (this.inciteTick == 10) {
                if (!this.getList().isEmpty()) {
                    for (Raider raider : this.getList()) {
