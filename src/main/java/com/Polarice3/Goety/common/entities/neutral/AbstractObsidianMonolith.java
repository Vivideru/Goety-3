...
public abstract class AbstractObsidianMonolith extends AbstractMonolith {
    protected static final EntityDataAccessor<Float> DATA_PROGRESS = SynchedEntityData.defineId(AbstractObsidianMonolith.class, EntityDataSerializers.FLOAT);
    protected static final EntityDataAccessor<Integer> DATA_LEVEL = SynchedEntityData.defineId(AbstractObsidianMonolith.class, EntityDataSerializers.INT);
    protected static final EntityDataAccessor<Integer> DATA_LEVELING = SynchedEntityData.defineId(AbstractObsidianMonolith.class, EntityDataSerializers.INT);
    public int startUp;
    ...
    public AbstractObsidianMonolith(EntityType<? extends AbstractMonolith> type, Level worldIn) {
        super(type, worldIn);
    }

    public static AttributeSupplier.Builder setCustomAttributes(){
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.ObsidianMonolithHealth))
                .add(Attributes.MOVEMENT_SPEED, 0.0D)
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.ObsidianMonolithArmor))
                .add(Attributes.KNOCKBACK_RESISTANCE, 1.0D)
                .add(Attributes.FOLLOW_RANGE, 32.0D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.ObsidianMonolithHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.ObsidianMonolithArmor));
    }

    @Override
    protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
        super.defineSynchedData(pBuilder);
        pBuilder.define(DATA_PROGRESS, 0.0F);
        pBuilder.define(DATA_LEVEL, 0);
        pBuilder.define(DATA_LEVELING, 0);
    }
            ...
            if (ModDamageSource.physicalAttacks(pSource)) {
                if (pSource.getDirectEntity() instanceof LivingEntity living) {
                    if (living.getMainHandItem().isCorrectToolForDrops(this.getState())){
                        damage = true;
                        efficiency += living.getMainHandItem().getEnchantmentLevel(living.level().registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(net.minecraft.world.item.enchantment.Enchantments.EFFICIENCY));
                    }
                }
            }
            if (damage){
            ...
            new SpellExplosion(this.level(), null, this.damageSources().magic(), this.blockPosition(), 16, 5.0F){
                ...
                public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z, double seen, float actualDamage) {
                    if (target instanceof IOwned owned && target instanceof LivingEntity living){
                        if (owned.getTrueOwner() == AbstractObsidianMonolith.this.getTrueOwner() || owned.getTrueOwner() == AbstractObsidianMonolith.this){
                            if (!(owned instanceof AbstractObsidianMonolith)) {
                                if (living.hurt(living.damageSources().magic(), AbstractObsidianMonolith.this.level().random.nextInt(10) + 5.0F)){
                                    AbstractObsidianMonolith.this.launch(living, AbstractObsidianMonolith.this);
                                }
                            }
                        }
                    }
    ...
    protected void dropAllDeathLoot(ServerLevel pLevel, DamageSource p_21192_) {
        if (ModDamageSource.physicalAttacks(p_21192_)) {
            if (p_21192_.getDirectEntity() instanceof LivingEntity living) {
                if (living.getMainHandItem().isCorrectToolForDrops(this.getState())){
                    super.dropAllDeathLoot(pLevel, p_21192_);
                }
            }
        }
    }
    ...
    public void ownerlessBehavior() {
        ...
        if (!this.level().isClientSide) {
            ...
            RandomSource random1 = this.level().getRandom();
            int spawnChance = 256;
            if (this.empowered > 0) {
                this.spreadNether();
                int width = 7;
            ...
            if (MobsConfig.ObsidianMonolithSpawner.get()) {
                ...
                if (serverLevel.getRandom().nextInt(spawnChance) == 0) {
                    if (j < 16){
                        ...
                        if (!spawners.isEmpty()) {
                            MobSpawnSettings.SpawnerData spawner = spawners.getRandom(serverLevel.getRandom()).orElse(null);
                            if (spawner != null) {
                                EntityType<?> entityType = spawner.type;
                                if (!MobUtil.hasEntityTypesConfig(MobsConfig.ObsidianMonolithBlackList.get(), entityType)) {
                                    ...
                                    if (SpawnPlacements.isSpawnPositionOk(entityType, this.level(), blockPos)
                                            && SpawnPlacements.checkSpawnRules(entityType, serverLevel, MobSpawnType.SPAWNER, blockPos, serverLevel.getRandom())) {
                                        if (entity instanceof Mob mob) {
                                            mob.setPos(blockPos.getX() + 0.5F, blockPos.getY(), blockPos.getZ() + 0.5F);
                                            EventHooks.finalizeMobSpawn(mob, serverLevel, serverLevel.getCurrentDifficultyAt(blockPos), MobSpawnType.SPAWNER, null);
                ...
                if (this.tickCount % 20 == 0) {
                    ...
                    if (this.spawnMiniBossCool <= 0 && j == 0 && heresiarchs == 0 && random1.nextFloat() <= 0.05F) {
                        ...
                        if (this.level().noCollision(heresiarch.getBoundingBox()) && this.level().getEntityCollisions(heresiarch, heresiarch.getBoundingBox()).isEmpty() && !this.level().containsAnyLiquid(heresiarch.getBoundingBox())) {
                            heresiarch.finalizeSpawn(serverLevel, this.level().getCurrentDifficultyAt(blockPos), MobSpawnType.SPAWNER, null);
    ...
    public void summonMavericks() {
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            if (this.level().noCollision(maverick.getBoundingBox()) && this.level().getEntityCollisions(maverick, maverick.getBoundingBox()).isEmpty() && !this.level().containsAnyLiquid(maverick.getBoundingBox())) {
                maverick.finalizeSpawn(serverLevel, this.level().getCurrentDifficultyAt(blockPos), MobSpawnType.MOB_SUMMONED, null);
    ...
    public void summonHeretics() {
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            if (this.level().noCollision(heretic.getBoundingBox()) && this.level().getEntityCollisions(heretic, heretic.getBoundingBox()).isEmpty() && !this.level().containsAnyLiquid(heretic.getBoundingBox())) {
                heretic.finalizeSpawn(serverLevel, this.level().getCurrentDifficultyAt(blockPos), MobSpawnType.MOB_SUMMONED, null);
    ...
    public void destroyBlocksAround() {
        if (this.destroyBlocksTick > 0) {
                ...
                for(int x1 = -3; x1 <= 3; ++x1) {
                    for(int y1 = 0; y1 <= 4; ++y1) {
                        for(int z1 = -3; z1 <= 3; ++z1) {
                            ...
                            if (!blockstate.is(BlockTags.WITHER_IMMUNE) && net.neoforged.neoforge.event.EventHooks.onEntityDestroyBlock(this, blockpos, blockstate)) {
