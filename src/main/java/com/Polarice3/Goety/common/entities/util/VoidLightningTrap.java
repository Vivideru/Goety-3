    ...
    public void finalizeAttack(){
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            if (this.getOwner() != null) {
                ModNetwork.sendToALL(new SLightningBoltPacket(this.getOwner().position().add(0, this.getOwner().getBbHeight(), 0), this.getOwner().position().add(0, 250, 0), colorUtil2, 10));
            }
