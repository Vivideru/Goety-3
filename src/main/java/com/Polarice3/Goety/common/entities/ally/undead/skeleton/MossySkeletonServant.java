...
public class MossySkeletonServant extends AbstractSkeletonServant {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.MossySkeletonServantHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.MossySkeletonServantArmor))
                .add(Attributes.MOVEMENT_SPEED, 0.25F)
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.get(AttributesConfig.MossySkeletonServantDamage));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.MossySkeletonServantHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.MossySkeletonServantArmor));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), AttributesConfig.get(AttributesConfig.MossySkeletonServantDamage));
    }

    public double getBaseRangeDamage(){
        return AttributesConfig.get(AttributesConfig.MossySkeletonServantRangeDamage);
    }
