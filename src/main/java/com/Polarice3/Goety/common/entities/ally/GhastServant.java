...
public class GhastServant extends Malghast {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.GhastServantHealth))
                .add(Attributes.FOLLOW_RANGE, 32.0D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.GhastServantHealth));
    }
        ...
        public void tick() {
            ...
            if (livingentity != null && livingentity.distanceToSqr(this.ghast) < Mth.square(d0) && this.ghast.hasLineOfSight(livingentity)) {
                ...
                if (this.chargeTime == 20) {
                    ...
                    largefireball.setDamage(MobUtil.getSpecialAttackDamage(this.ghast, AttributesConfig.get(AttributesConfig.GhastServantDamage).floatValue() + this.ghast.getFireBallDamage()));
                    largefireball.setPos(this.ghast.getX() + vec3.x() * 4.0D, this.ghast.getY() + 0.25D, this.ghast.getZ() + vec3.z() * 4.0D);
                    largefireball.setDangerous(EventHooks.canEntityGrief(world, this.ghast));
