...
public class EffectBlastTrap extends Entity implements ISpellEntity {
    ...
    protected void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("Potion", 8)) {
            this.setPotion(BuiltInRegistries.POTION.getHolder(ResourceLocation.parse(compound.getString("Potion"))).<Holder<Potion>>map(holder -> holder).orElse(Potions.WATER));
        }
    ...
    protected void addAdditionalSaveData(CompoundTag compound) {
        ...
        if (this.potion != Potions.WATER) {
            compound.putString("Potion", BuiltInRegistries.POTION.getKey(this.potion.value()).toString());
        }
    ...
    public void tick() {
        ...
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            for (int k1 = 0; (float) k1 < f5; ++k1) {
                ...
                serverLevel.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, (float)d5, (float)d6, (float)d7), this.getX() + (double) f8, this.getY(), this.getZ() + (double) f9, 0, 0.0D, 0.0D, 0.0D, 1.0F);
            }
            ...
            if (this.tickCount == 20 || this.getImmediate()){
                ...
                this.potion.value().getEffects().forEach(effect -> list.add(new MobEffectInstance(effect)));
