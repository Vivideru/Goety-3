...
public class ModWitherSkull extends WitherSkull {
      ...
      if (this.level() instanceof ServerLevel serverLevel) {
         ...
         if (entity1 instanceof LivingEntity livingentity) {
            if (entity1 instanceof Player){
               damage = SpellConfig.get(SpellConfig.WitherSkullDamage).floatValue() * WandUtil.damageMultiply();
            } else {
               damage = this.getDamage();
            }
            DamageSource damageSource = this.damageSources().witherSkull(this, livingentity);
            flag = entity.hurt(damageSource, damage + enchantment);
            if (flag) {
               if (entity.isAlive()) {
         ...
         if (flag && entity instanceof LivingEntity livingEntity) {
            if (livingEntity instanceof Player player){
               if (WandUtil.enchantedFocus(player)){
                  duration += WandUtil.getLevels(ModEnchantments.DURATION, player);
               }
            }
   ...
   protected void onHit(HitResult pResult) {
      ...
      if (!this.level().isClientSide) {
         ...
         Explosion.BlockInteraction explodeMode = net.neoforged.neoforge.event.EventHooks.canEntityGrief(this.level(), this) ? Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.KEEP;
         if (this.getOwner() instanceof Player) {
            explodeMode = SpellConfig.get(SpellConfig.WitherSkullGriefing) ? Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.KEEP;
         }
