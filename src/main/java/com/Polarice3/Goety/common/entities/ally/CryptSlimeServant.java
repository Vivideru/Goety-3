...
public class CryptSlimeServant extends SlimeServant implements IMobTyped{
    ...
    protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource p_33574_, boolean p_33576_) {
        ...
        if (serverLevel.getServer() != null) {
            LootTable loottable = serverLevel.getServer().reloadableRegistries().getLootTable(ResourceKey.create(Registries.LOOT_TABLE, ModLootTables.CRYPT_SLIME));
        ...
        if (this.isAlive()) {
            ...
            if (this.distanceToSqr(livingEntity) < 0.6D * (double)i * 0.6D * (double)i && this.hasLineOfSight(livingEntity) && livingEntity.hurt(this.getServantAttack(), this.getAttackDamage())) {
                ...
                if (this.level() instanceof ServerLevel serverLevel) {
                    EnchantmentHelper.doPostAttackEffects(serverLevel, livingEntity, this.getServantAttack());
                }
