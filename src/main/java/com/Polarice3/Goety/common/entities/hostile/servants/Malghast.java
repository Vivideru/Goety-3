...
public class Malghast extends SummonedFlying implements IMobTyped {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.MalghastHealth))
                .add(Attributes.FOLLOW_RANGE, 32.0D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.MalghastHealth));
    }
                ...
                if (this.chargeTime == 20) {
                    ...
                    if (this.shotTimes) {
                        ...
                        if (fireballentity instanceof HellBlast hellBlast){
                            hellBlast.setDamage(AttributesConfig.get(AttributesConfig.MalghastDamage).floatValue() + this.ghast.getFireBallDamage());
                        }
                        charge = -20;
                    } else {
                        fireballentity = new Lavaball(world, this.ghast, d2, d3, d4);
                        if (fireballentity instanceof Lavaball lavaball){
                            lavaball.setDamage(AttributesConfig.get(AttributesConfig.MalghastDamage).floatValue() + this.ghast.getFireBallDamage());
                            lavaball.setExplosionPower(power);
                            lavaball.setDangerous(EventHooks.canEntityGrief(world, this.ghast));
                        }
                    }
