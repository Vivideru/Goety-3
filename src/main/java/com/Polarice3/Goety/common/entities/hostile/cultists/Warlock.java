...
public class Warlock extends Cultist implements RangedAttackMob {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, AttributesConfig.get(AttributesConfig.WarlockHealth))
                .add(Attributes.ARMOR, AttributesConfig.get(AttributesConfig.WarlockArmor))
                .add(Attributes.MOVEMENT_SPEED, 0.25D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), AttributesConfig.get(AttributesConfig.WarlockHealth));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), AttributesConfig.get(AttributesConfig.WarlockArmor));
    }
    ...
    private void summonWartlings(Wartling wartling){
        if (this.level() instanceof ServerLevel serverLevel) {
            // Summons spawned by an owned Warlock should belong to the same effective owner.
