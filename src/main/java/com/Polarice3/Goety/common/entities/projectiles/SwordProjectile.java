...
public class SwordProjectile extends AbstractArrow implements ItemSupplier {
    ...
    public void setItem(ItemStack pStack) {
        if (pStack.getItem() != this.getDefaultItem() || !pStack.getComponentsPatch().isEmpty()) {
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (!this.getItem().isEmpty()) {
            f1 += MobUtil.getItemEnchantmentLevel(this, this.getItem(), Enchantments.KNOCKBACK);
            i = MobUtil.getItemEnchantmentLevel(this, this.getItem(), Enchantments.FIRE_ASPECT);
        }
        ...
        if (target instanceof LivingEntity livingentity) {
            f = MobUtil.getEnchantedDamage(this, this.getItem(), livingentity, damagesource, f);
        }
        SoundEvent soundevent = SoundEvents.TRIDENT_HIT;
        if (target.hurt(damagesource, f)) {
            if (target instanceof LivingEntity livingTarget) {
                if (owner instanceof LivingEntity) {
                    MobUtil.doPostAttackEffects(this, livingTarget, damagesource, this.getItem());
