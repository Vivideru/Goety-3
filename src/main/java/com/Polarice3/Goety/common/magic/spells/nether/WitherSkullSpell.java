...
public class WitherSkullSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        float extraBlast = (float) (spellStat.getRadius() + WandUtil.getLevels(ModEnchantments.RADIUS, caster) / 2.0F);
        ...
        witherSkull.setFiery(spellStat.getBurning() + WandUtil.getLevels(ModEnchantments.BURNING, caster));
        ...
        if (rightStaff(staff)) {
            for (int i = 0; i < 2; ++i) {
                ...
                witherSkull1.setFiery(spellStat.getBurning() + WandUtil.getLevels(ModEnchantments.BURNING, caster));
