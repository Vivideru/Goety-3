                ...
                if (SpellConfig.DragonFrostGriefing.get()) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (hitResult.getType() == HitResult.Type.BLOCK) {
                            ...
                            if (worldIn.getBlockState(blockPos) == FrostedIceBlock.meltsInto() && blockstate.canSurvive(worldIn, blockPos) && worldIn.isUnobstructed(blockstate, blockPos, CollisionContext.empty()) && !net.neoforged.neoforge.event.EventHooks.onBlockPlace(caster, net.neoforged.neoforge.common.util.BlockSnapshot.create(worldIn.dimension(), worldIn, blockPos), net.minecraft.core.Direction.UP)) {
            ...
            for (Entity target : getBreathTarget(caster, range)) {
                ...
                if (livingEntity != null) {
                    if (livingEntity.hurt(ModDamageSource.frostBreath(caster, caster), damage)) {
                        livingEntity.addEffect(new MobEffectInstance(GoetyEffects.FREEZING, MathHelper.secondsToTicks(1) * duration));
                    }
                }
            }
        }
