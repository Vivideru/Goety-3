...
public class EndWalkSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (WandUtil.enchantedFocus(caster)){
            ...
            duration = MathHelper.secondsToTicks(5) * WandUtil.getLevels(ModEnchantments.DURATION, caster);
        }
        if (caster instanceof Player player){
            player.addEffect(new MobEffectInstance(GoetyEffects.SHADOW_WALK, SpellConfig.EndWalkEffectDuration.get() + duration, potency, false, false, true));
            for(int i = 0; i < 16; ++i) {
