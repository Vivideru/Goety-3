...
public class ChargeSpell extends TouchSpell {
    ...
    public void touchResult(ServerLevel worldIn, LivingEntity caster, LivingEntity target, ItemStack staff, SpellStat spellStat) {
        if (this.rightStaff(staff)) {
            ...
            if (!target.hasEffect(GoetyEffects.CHARGED)) {
                this.playSound(worldIn, target, 1.0F, 0.5F);
                target.addEffect(new MobEffectInstance(GoetyEffects.CHARGED, MathHelper.secondsToTicks(30), 0, false, false));
            } else {
                MobEffectInstance instance = target.getEffect(GoetyEffects.CHARGED);
                if (instance != null) {
