    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                ...
                spriteMob.setTrueOwner(MobUtil.getSummonOwner(caster));
                spriteMob.moveTo(blockpos, caster.getYRot(), 0.0F);
                spriteMob.setLimitedLife(MobUtil.getSummonLifespan(worldIn) * duration);
                spriteMob.finalizeSpawn(worldIn, caster.level().getCurrentDifficultyAt(blockpos), MobSpawnType.MOB_SUMMONED, null);
