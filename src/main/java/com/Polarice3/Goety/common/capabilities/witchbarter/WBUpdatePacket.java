...
public class WBUpdatePacket {
    ...
    public WBUpdatePacket(LivingEntity livingEntity) {
        ...
        livingEntity.getExistingData(WitchBarterProvider.CAPABILITY).ifPresent((barter) -> {
    ...
    public static void consume(WBUpdatePacket packet, Supplier<ModNetwork.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            if (ctx.get().getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
                ...
                if (level instanceof ClientLevel clientLevel) {
                    ...
                    if (entity != null) {
                        entity.getExistingData(WitchBarterProvider.CAPABILITY).ifPresent((barter) -> {
