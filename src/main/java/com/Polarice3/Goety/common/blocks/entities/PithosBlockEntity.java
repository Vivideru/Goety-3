...
public class PithosBlockEntity extends RandomizableContainerBlockEntity implements IChunkLoader {
    ...
    protected void saveAdditional(CompoundTag p_187459_, HolderLookup.Provider provider) {
        super.saveAdditional(p_187459_, provider);
        if (!this.trySaveLootTable(p_187459_)) {
            ContainerHelper.saveAllItems(p_187459_, this.items, provider);
        }
        if (this.hasCustomSLName()) {
            p_187459_.putString("SkullLordName", Component.Serializer.toJson(this.skullLordName, provider));
        }
    }

    protected void loadAdditional(CompoundTag p_155055_, HolderLookup.Provider provider) {
        super.loadAdditional(p_155055_, provider);
        this.items = NonNullList.withSize(this.getContainerSize(), ItemStack.EMPTY);
        if (!this.tryLoadLootTable(p_155055_)) {
            ContainerHelper.loadAllItems(p_155055_, this.items, provider);
        }
        if (p_155055_.contains("SkullLordName")) {
            this.skullLordName = Component.Serializer.fromJson(p_155055_.getString("SkullLordName"), provider);
