...
public class NecroBrazierBlockEntity extends ModBlockEntity implements Clearable {
    ...
    private CraftingInput getRecipeInput() {
        return CraftingInput.of(this.getContainer().getContainerSize(), 1, this.getItems());
    }
    ...
    public void readNetwork(CompoundTag compoundNBT) {
        ...
        if (this.level != null) {
            ContainerHelper.loadAllItems(compoundNBT, items, this.level.registryAccess());
        }
        this.setItems(items);
        this.currentTime = compoundNBT.getInt("currentTime");
    }

    public void readNetwork(CompoundTag compoundNBT, net.minecraft.core.HolderLookup.Provider provider) {
        NonNullList<ItemStack> items = NonNullList.withSize(this.getContainer().getContainerSize(), ItemStack.EMPTY);
        ContainerHelper.loadAllItems(compoundNBT, items, provider);
        this.setItems(items);
        this.currentTime = compoundNBT.getInt("currentTime");
    }
    ...
    public CompoundTag writeNetwork(CompoundTag pCompound, net.minecraft.core.HolderLookup.Provider provider) {
        this.saveMetadataAndItems(pCompound, provider);
        pCompound.putInt("currentTime", this.currentTime);
        return pCompound;
    }
    ...
    private CompoundTag saveMetadataAndItems(CompoundTag pCompound) {
        if (this.level != null) {
            ContainerHelper.saveAllItems(pCompound, this.getItems(), true, this.level.registryAccess());
        }
        return pCompound;
    }

    private CompoundTag saveMetadataAndItems(CompoundTag pCompound, net.minecraft.core.HolderLookup.Provider provider) {
        ContainerHelper.saveAllItems(pCompound, this.getItems(), true, provider);
        return pCompound;
    }
    ...
    public boolean activate(Level world) {
        if (this.getRecipe() == null) {
            BrazierRecipe brazierRecipe = world.getRecipeManager().getAllRecipesFor(ModRecipeSerializer.BRAZIER_TYPE.get()).stream().map(holder -> holder.value()).filter(
    ...
    public BrazierRecipe getRecipe(){
        if(this.recipeId != null){
            if(this.level != null) {
                Optional<? extends Recipe<?>> recipe = this.level.getRecipeManager().byKey(this.recipeId).map(holder -> holder.value());
        ...
        if (this.level != null) {
            if (!this.level.isClientSide) {
                ...
                if (recipe != null) {
                    ...
                    if (finished) {
                        CraftingInput input = this.getRecipeInput();
                        ItemStack itemstack1 = this.level.getRecipeManager()
                                .getRecipeFor(ModRecipeSerializer.BRAZIER_TYPE.get(), input, this.level)
                                .map((recipes) -> recipes.value().assemble(input, this.level.registryAccess())).orElse(ItemStack.EMPTY);
