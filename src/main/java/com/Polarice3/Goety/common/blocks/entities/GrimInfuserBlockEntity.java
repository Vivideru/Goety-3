...
public class GrimInfuserBlockEntity extends ModBlockEntity implements Clearable, WorldlyContainer {
    ...
    public void readNetwork(CompoundTag compoundNBT) {
        this.items.clear();
        if (this.level != null) {
            ContainerHelper.loadAllItems(compoundNBT, this.items, this.level.registryAccess());
        }
    ...
    public void readNetwork(CompoundTag compoundNBT, net.minecraft.core.HolderLookup.Provider provider) {
        this.items.clear();
        ContainerHelper.loadAllItems(compoundNBT, this.items, provider);
        if (compoundNBT.contains("CookingTimes", 11)) {
            int[] aint = compoundNBT.getIntArray("CookingTimes");
            System.arraycopy(aint, 0, this.cookingProgress, 0, Math.min(this.cookingTime.length, aint.length));
        }
    ...
    private CompoundTag saveMetadataAndItems(CompoundTag pCompound) {
        if (this.level != null) {
            ContainerHelper.saveAllItems(pCompound, this.items, true, this.level.registryAccess());
        }
        return pCompound;
    }

    private CompoundTag saveMetadataAndItems(CompoundTag pCompound, net.minecraft.core.HolderLookup.Provider provider) {
        ContainerHelper.saveAllItems(pCompound, this.items, true, provider);
        return pCompound;
    }
    ...
    public Optional<CursedInfuserRecipes> getRecipes(ItemStack pStack) {
        return this.items.stream().noneMatch(ItemStack::isEmpty) ? Optional.empty() : this.level.getRecipeManager().getRecipeFor(ModRecipeSerializer.CURSED_INFUSER.get(), new SingleRecipeInput(pStack), this.level).map(holder -> holder.value());
    }
