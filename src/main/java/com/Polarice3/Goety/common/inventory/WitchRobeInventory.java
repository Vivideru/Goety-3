...
public class WitchRobeInventory extends SimpleContainer implements MenuProvider {
    ...
    public WitchRobeInventory(CompoundTag nbt, HolderLookup.Provider provider) {
    ...
    private void doBrew() {
        if (net.neoforged.neoforge.event.EventHooks.onPotionAttemptBrew(items)) {
            return;
        }
        ItemStack itemstack = this.items.get(3);
    ...
    public boolean isWaterOrEmpty(){
        ...
        if (!inputEmpty()) {
            flag[0] = ModPotionUtil.getPotion(this.items.get(0)) == Potions.WATER || isAirOrEmpty(this.items.get(0));
            flag[1] = ModPotionUtil.getPotion(this.items.get(1)) == Potions.WATER || isAirOrEmpty(this.items.get(1));
            flag[2] = ModPotionUtil.getPotion(this.items.get(2)) == Potions.WATER || isAirOrEmpty(this.items.get(2));
        }
    ...
    public void autoAddWaterBottles(ItemStack itemStack){
        if (itemStack.is(Items.GLASS_BOTTLE)){
            if (this.inputEmpty()){
                this.addBottles(itemStack, ModPotionUtil.setPotion(new ItemStack(Items.POTION), Potions.WATER));
    ...
    public void load(CompoundTag p_230337_2_, HolderLookup.Provider provider) {
    ...
    public boolean canPlaceItem(int pIndex, ItemStack pStack) {
        if (pIndex == 3) {
            return this.getPotionBrewing().isIngredient(pStack);
        } else {
            Item item = pStack.getItem();
            if (pIndex == 4) {
                return item == Items.BLAZE_POWDER;
            } else {
                return this.getPotionBrewing().isInput(pStack) && this.getItem(pIndex).isEmpty();
            }
        }
    }
