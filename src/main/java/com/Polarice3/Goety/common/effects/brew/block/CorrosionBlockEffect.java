...
public class CorrosionBlockEffect extends BrewEffect {
        ...
        if (pLevel instanceof ServerLevel serverLevel) {
            for (BlockPos blockPos : this.getSpherePos(pPos, pAreaOfEffect + 3)) {
                ...
                if (!state.is(BlockTags.WITHER_IMMUNE)
                    ...
                    serverLevel.destroyBlock(blockPos, state.is(Blocks.OBSIDIAN));
                }
    ...
    public boolean canEntityBreak(Level pLevel, BlockState state, BlockPos blockPos, LivingEntity pSource) {
        if (pSource == null) {
            ...
            return state.canEntityDestroy(pLevel, blockPos, pSource) && EventHooks.onEntityDestroyBlock(pSource, blockPos, state);
        }
    }
