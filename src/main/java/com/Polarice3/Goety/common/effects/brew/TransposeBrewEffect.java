            ...
            for (Entity entity : pLevel.getEntitiesOfClass(Entity.class, new AABB(blockPos))){
                if (entity instanceof ItemEntity || entity instanceof LivingEntity) {
                    if (entity instanceof LivingEntity livingEntity) {
                        net.neoforged.neoforge.event.entity.EntityTeleportEvent.EnderEntity event = net.neoforged.neoforge.event.EventHooks.onEnderTeleport(livingEntity, pPos.getX(), pPos.getY() + 1, pPos.getZ());
                        if (event.isCanceled()) {
                            break;
                        }
                    }
