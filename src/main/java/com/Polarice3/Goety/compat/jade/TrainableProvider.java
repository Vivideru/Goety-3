...
public enum TrainableProvider implements IEntityComponentProvider, IServerDataProvider<EntityAccessor> {
    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
        ...
        if (time > 0 && totalTime > 0) {
            tooltip.add(Component.translatable("jade.goety.training", IThemeHelper.get().seconds(time, TICKS_PER_SECOND), IThemeHelper.get().seconds(totalTime, TICKS_PER_SECOND)));
        }
    }
