...
public class HeresiarchRenderer<T extends Heresiarch> extends MobRenderer<T, HeresiarchModel<T>> {
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        vertex(vertexconsumer, matrix4f, LightTexture.FULL_BRIGHT, 0.0F, 0, 0, 1);
        vertex(vertexconsumer, matrix4f, LightTexture.FULL_BRIGHT, 1.0F, 0, 1, 1);
        vertex(vertexconsumer, matrix4f, LightTexture.FULL_BRIGHT, 1.0F, 1, 1, 0);
        vertex(vertexconsumer, matrix4f, LightTexture.FULL_BRIGHT, 0.0F, 1, 0, 0);
        poseStack.popPose();
        if (entity.isAlive()) {
            ...
            for (int i = 0; i < entity.trailSnapshots.size(); i++) {
                ...
                if (modelAlpha > 0) {
                    this.shadowModel.renderToBuffer(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY, ((int)(modelAlpha * 255.0F) << 24) | 0x00FFFFFF);
                }
                poseStack.popPose();
            }
        }
    ...
    private static void vertex(VertexConsumer p_254095_, Matrix4f p_254477_, int p_253829_, float p_253995_, int p_254031_, int p_253641_, int p_254243_) {
        p_254095_.addVertex(p_254477_, p_253995_ - 0.5F, (float)p_254031_ - 0.5F, 0.0F)
