...
public class HierarchicalArmorLayer<T extends LivingEntity, M extends EntityModel<T> & HierarchicalArmor> extends RenderLayer<T, M> {
    ...
    public static ResourceLocation getArmorResource(Entity entity, ItemStack stack, EquipmentSlot slot, @Nullable String type) {
        ...
        for (ArmorMaterial.Layer candidate : material.layers()) {
            if ((type == null && candidate.dyeable()) || (type != null && !candidate.dyeable())) {
                layer = candidate;
                break;
            }
        }
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, LivingEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        {
            ...
            if (headItem.getItem() instanceof ArmorItem armoritem) {
                if (headItem.canEquip(EquipmentSlot.HEAD, entity)) {
                    ...
                    if (armoritem.getMaterial().value().layers().stream().anyMatch(ArmorMaterial.Layer::dyeable)) { // Allow this for anything, not only cloth
                        int i = DyedItemColor.getOrDefault(headItem, DyedItemColor.LEATHER_COLOR);
        ...
        }
        {
            ...
            if (chestItem.getItem() instanceof ArmorItem armoritem) {
                if (armoritem.getEquipmentSlot() == EquipmentSlot.CHEST) {
                    ...
                    int clampedLight = packedLightIn;
                    if (armoritem.getMaterial().value().layers().stream().anyMatch(ArmorMaterial.Layer::dyeable)) { // Allow this for anything, not only cloth
                        int i = DyedItemColor.getOrDefault(chestItem, DyedItemColor.LEATHER_COLOR);
                        float f = (float) (i >> 16 & 255) / 255.0F;
                        float f1 = (float) (i >> 8 & 255) / 255.0F;
                        float f2 = (float) (i & 255) / 255.0F;
        ...
        }
        {
            ...
            if (legItem.getItem() instanceof ArmorItem armoritem) {
                if (armoritem.getEquipmentSlot() == EquipmentSlot.LEGS) {
                    ...
                    int clampedLight = packedLightIn;
                    if (armoritem.getMaterial().value().layers().stream().anyMatch(ArmorMaterial.Layer::dyeable)) { // Allow this for anything, not only cloth
                        int i = DyedItemColor.getOrDefault(legItem, DyedItemColor.LEATHER_COLOR);
                        float f = (float) (i >> 16 & 255) / 255.0F;
                        float f1 = (float) (i >> 8 & 255) / 255.0F;
                        float f2 = (float) (i & 255) / 255.0F;
        ...
        }
        {
            ...
            if (feetItem.getItem() instanceof ArmorItem armoritem) {
                if (armoritem.getEquipmentSlot() == EquipmentSlot.FEET) {
                    ...
                    int clampedLight = packedLightIn;
                    if (armoritem.getMaterial().value().layers().stream().anyMatch(ArmorMaterial.Layer::dyeable)) { // Allow this for anything, not only cloth
                        int i = DyedItemColor.getOrDefault(feetItem, DyedItemColor.LEATHER_COLOR);
                        float f = (float) (i >> 16 & 255) / 255.0F;
                        float f1 = (float) (i >> 8 & 255) / 255.0F;
                        float f2 = (float) (i & 255) / 255.0F;
    ...
    private static int colorFromRgb(float red, float green, float blue) {
        return 0xFF000000 | ((int)(red * 255.0F) << 16) | ((int)(green * 255.0F) << 8) | (int)(blue * 255.0F);
    }

    private void renderTrim(ModelPart part, Holder<ArmorMaterial> armorMaterial, PoseStack poseStack, MultiBufferSource pBuffer, int packedLight, ArmorTrim armorTrim, boolean glintIn, boolean innerModel, float red, float green, float blue) {
        ...
        VertexConsumer vertexconsumer = textureatlassprite.wrap(ItemRenderer.getFoilBufferDirect(pBuffer, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
        part.render(poseStack, vertexconsumer, packedLight, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
    }
    ...
    private void renderLeg(ItemStack legItem, LivingEntity entity, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean glintIn, HumanoidModel modelIn, float red, float green, float blue, ResourceLocation armorResource, boolean notAVanillaModel) {
        ...
        renderer.getModel().rightLegPartArmors().forEach(part -> {
            ...
            modelIn.rightLeg.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        renderer.getModel().leftLegPartArmors().forEach(part -> {
            matrixStackIn.pushPose();
            renderer.getModel().translateToLeg(part, matrixStackIn);
            modelIn.leftLeg.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        renderer.getModel().bodyPartArmors().forEach(part -> {
            matrixStackIn.pushPose();
            renderer.getModel().translateToChest(part, matrixStackIn);
            modelIn.body.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        if (MobsConfig.HierarchicalArmorTrim.get()) {
            ArmorTrim armorTrim = legItem.get(DataComponents.TRIM); if (armorTrim != null) {
                if (legItem.getItem() instanceof ArmorItem armorItem) {
                    TextureAtlasSprite sprite = this.armorTrimAtlas.getSprite(armorTrim.innerTexture(armorItem.getMaterial()));
                    renderer.getModel().rightLegPartArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToLeg(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.rightLeg.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                    renderer.getModel().leftLegPartArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToLeg(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.leftLeg.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                    renderer.getModel().bodyPartArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToChest(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.body.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                }
            }
    ...
    private void renderBoot(ItemStack feetItem, LivingEntity entity, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean glintIn, HumanoidModel modelIn, float red, float green, float blue, ResourceLocation armorResource, boolean notAVanillaModel) {
        ...
        if (MobsConfig.HierarchicalArmorTrim.get()) {
            ArmorTrim armorTrim = feetItem.get(DataComponents.TRIM); if (armorTrim != null) {
                if (feetItem.getItem() instanceof ArmorItem armorItem) {
    ...
    private void renderChestplate(ItemStack chestItem, LivingEntity entity, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean glintIn, HumanoidModel modelIn, float red, float green, float blue, ResourceLocation armorResource, boolean notAVanillaModel) {
        ...
        renderer.getModel().rightHandArmors().forEach(part -> {
            ...
            modelIn.rightArm.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        renderer.getModel().leftHandArmors().forEach(part -> {
            matrixStackIn.pushPose();
            renderer.getModel().translateToArms(part, matrixStackIn);
            modelIn.leftArm.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        renderer.getModel().bodyPartArmors().forEach(part -> {
            matrixStackIn.pushPose();
        ...
        if (MobsConfig.HierarchicalArmorTrim.get()) {
            ArmorTrim armorTrim = chestItem.get(DataComponents.TRIM); if (armorTrim != null) {
                if (chestItem.getItem() instanceof ArmorItem armorItem) {
                    TextureAtlasSprite sprite = this.armorTrimAtlas.getSprite(armorTrim.outerTexture(armorItem.getMaterial()));
                    renderer.getModel().rightHandArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToArms(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.rightArm.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                    renderer.getModel().leftHandArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToArms(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.leftArm.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                    renderer.getModel().bodyPartArmors().forEach(part -> {
                        matrixStackIn.pushPose();
    ...
    private void renderHelmet(ItemStack headItem, LivingEntity entity, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, boolean glintIn, HumanoidModel modelIn, float red, float green, float blue, ResourceLocation armorResource, boolean notAVanillaModel) {
        ...
        renderer.getModel().headPartArmors().forEach(part -> {
            ...
            modelIn.head.render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, colorFromRgb(red, green, blue));
            matrixStackIn.popPose();
        });
        if (MobsConfig.HierarchicalArmorTrim.get()) {
            ArmorTrim armorTrim = headItem.get(DataComponents.TRIM); if (armorTrim != null) {
                if (headItem.getItem() instanceof ArmorItem armorItem) {
                    TextureAtlasSprite sprite = this.armorTrimAtlas.getSprite(armorTrim.outerTexture(armorItem.getMaterial()));
                    renderer.getModel().headPartArmors().forEach(part -> {
                        matrixStackIn.pushPose();
                        renderer.getModel().translateToHead(part, matrixStackIn);
                        VertexConsumer consumer = sprite.wrap(ItemRenderer.getFoilBufferDirect(bufferIn, Sheets.armorTrimsSheet(armorTrim.pattern().value().decal()), false, glintIn));
                        modelIn.head.render(matrixStackIn, consumer, packedLightIn, OverlayTexture.NO_OVERLAY, -1);
                        matrixStackIn.popPose();
                    });
                }
            }
