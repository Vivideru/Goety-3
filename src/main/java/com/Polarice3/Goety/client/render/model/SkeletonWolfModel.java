...
public class SkeletonWolfModel<T extends SkeletonWolf> extends HierarchicalModel<T> {
    ...
    public void renderToBuffer(PoseStack p_102424_, VertexConsumer p_102425_, int p_102426_, int p_102427_, int color) {
        ...
        int tintedColor = FastColor.ARGB32.color(
                ...
                Math.round(this.g * FastColor.ARGB32.green(color)),
                Math.round(this.b * FastColor.ARGB32.blue(color)));
        if (this.young) {
            p_102424_.pushPose();
            p_102424_.translate(0.0F, 1.75F, 4.0F / 16.0F);
            this.headParts().forEach((p_102081_) -> {
                p_102081_.render(p_102424_, p_102425_, p_102426_, p_102427_, tintedColor);
            });
            p_102424_.popPose();
            p_102424_.pushPose();
            float f1 = 1.0F / 2.0F;
            ...
            this.bodyParts().forEach((p_102071_) -> {
                p_102071_.render(p_102424_, p_102425_, p_102426_, p_102427_, tintedColor);
            });
            p_102424_.popPose();
        } else {
            super.renderToBuffer(p_102424_, p_102425_, p_102426_, p_102427_, tintedColor);
        }
    }
