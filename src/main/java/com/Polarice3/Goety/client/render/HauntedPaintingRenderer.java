...
public class HauntedPaintingRenderer extends EntityRenderer<HauntedPainting> {
   ...
   public void render(HauntedPainting entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
      ...
      try (var ignored = new CloseablePoseStack(poseStack)) {
         ...
         model.renderToBuffer(poseStack, buffer.getBuffer(RenderType.entitySolid(frameTexture)), packedLight, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
   ...
   private static void renderPainting(PoseStack poseStack, VertexConsumer consumer, Direction dir, int light, int overlay) {
      ...
      consumer.addVertex(poseStack.last().pose(), 0, 0, 0).setColor(255, 255, 255, 255).setUv(0, 0).setOverlay(overlay).setLight(light).setNormal(poseStack.last(), normal.getX(), normal.getY(), normal.getZ());
      consumer.addVertex(poseStack.last().pose(), 0, 1, 0).setColor(255, 255, 255, 255).setUv(0, 1).setOverlay(overlay).setLight(light).setNormal(poseStack.last(), normal.getX(), normal.getY(), normal.getZ());
      consumer.addVertex(poseStack.last().pose(), -1, 1, 0).setColor(255, 255, 255, 255).setUv(1, 1).setOverlay(overlay).setLight(light).setNormal(poseStack.last(), normal.getX(), normal.getY(), normal.getZ());
      consumer.addVertex(poseStack.last().pose(), -1, 0, 0).setColor(255, 255, 255, 255).setUv(1, 0).setOverlay(overlay).setLight(light).setNormal(poseStack.last(), normal.getX(), normal.getY(), normal.getZ());
   }
