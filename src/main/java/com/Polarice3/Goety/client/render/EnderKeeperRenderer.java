...
public class EnderKeeperRenderer<T extends EnderKeeper> extends MobRenderer<T, EnderKeeperModel<T>> {
    ...
    public void render(T pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (pEntity.deathTime > MathHelper.secondsToTicks(2.5F)){
            ...
            this.setupRotations(pEntity, pMatrixStack, f71, f, pPartialTicks, pEntity.getScale());
            ...
            this.model.renderToBuffer(pMatrixStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, FastColor.ARGB32.color((int)(Mth.clamp(f9, 0.0F, 1.0F) * 255.0F), 255, 255, 255));
            VertexConsumer ivertexbuilder1 = pBuffer.getBuffer(RenderType.entityDecal(this.getTextureLocation(pEntity)));
            int tint = (int)(Mth.clamp(f10, 0.0F, 1.0F) * 255.0F);
            this.model.renderToBuffer(pMatrixStack, ivertexbuilder1, pPackedLight, OverlayTexture.pack(0.0F, flag), FastColor.ARGB32.color(255, tint, tint, tint));
        ...
        if (pEntity.isAlive()) {
            ...
            for (int i = 0; i < pEntity.trailSnapshots.size(); i++) {
                ...
                if (modelAlpha > 0) {
                    this.shadowModel.renderToBuffer(pMatrixStack, vertexConsumer, pPackedLight, OverlayTexture.NO_OVERLAY, FastColor.ARGB32.color((int)(Mth.clamp(modelAlpha, 0.0F, 1.0F) * 255.0F), 255, 255, 255));
                }
                pMatrixStack.popPose();
            }
        }
