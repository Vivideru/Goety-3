...
public class WightRenderer<T extends Wight> extends MobRenderer<T, WightModel<T>> {
    ...
    public void render(T pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (pEntity.deathTime > 0){
            ...
            this.model.renderToBuffer(pMatrixStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, net.minecraft.util.FastColor.ARGB32.color(Mth.floor(f9 * 255.0F), 255, 255, 255));
            VertexConsumer ivertexbuilder1 = pBuffer.getBuffer(RenderType.entityDecal(this.getTextureLocation(pEntity)));
            int color = net.minecraft.util.FastColor.ARGB32.color(255, Mth.floor(f10 * 255.0F), Mth.floor(f10 * 255.0F), Mth.floor(f10 * 255.0F));
            this.model.renderToBuffer(pMatrixStack, ivertexbuilder1, pPackedLight, OverlayTexture.pack(0.0F, flag), color);
