...
public class WaterJetRenderer {
    ...
    private static void renderJetLayer(VertexConsumer consumer, PoseStack poseStack, float radius, float length, float textureRatio, float textureOffset) {
        ...
        consumer.addVertex(pose.pose(), -radius, 0, -radius).setColor(-1).setUv(textureOffset, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(0, 1, 0);
        consumer.addVertex(pose.pose(), -radius, 0, radius).setColor(-1).setUv(textureOffset, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(0, 1, 0);
        consumer.addVertex(pose.pose(), -radius, length, radius).setColor(-1).setUv(uEnd + textureOffset, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(0, 1, 0);
        consumer.addVertex(pose.pose(), -radius, length, -radius).setColor(-1).setUv(uEnd + textureOffset, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightTexture.FULL_BRIGHT).setNormal(0, 1, 0);
