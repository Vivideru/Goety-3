...
public class SpirallingParticleOption implements ParticleOptions {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, SpirallingParticleOption> STREAM_CODEC = StreamCodec.of(
           ...
           SpirallingParticleOption::fromNetwork
   );

   private static SpirallingParticleOption fromNetwork(RegistryFriendlyByteBuf buffer) {
            return new SpirallingParticleOption(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readInt());
   }
}
