...
public class SphereExplodeParticleOption implements ParticleOptions {
    public static final MapCodec<SphereExplodeParticleOption> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, SphereExplodeParticleOption> STREAM_CODEC = StreamCodec.of(
           (buffer, option) -> option.writeToNetwork(buffer),
           SphereExplodeParticleOption::fromNetwork
   );

   private static SphereExplodeParticleOption fromNetwork(RegistryFriendlyByteBuf buffer) {
            return new SphereExplodeParticleOption(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readInt());
   }
