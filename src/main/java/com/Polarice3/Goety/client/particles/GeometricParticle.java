...
/**
 * Adapted from @cerbon's Cerbon API code:<a href="https://github.com/CERBON-MODS/CERBONs-API/blob/master/Common/src/main/java/com/cerbon/cerbons_api/api/general/particle/SimpleParticle.java">...</a>
 */
public class GeometricParticle extends TextureSheetParticle {
    ...
    public void render(VertexConsumer vertexConsumer, @NotNull Camera camera, float partialTicks) {
        ...
        vertexConsumer.addVertex(
                ...
                vector3fs[0].z()
        ).setUv(m, o).setColor(this.rCol, this.gCol, this.bCol, this.alpha).setLight((int) p);

        vertexConsumer.addVertex(
                vector3fs[1].x(), vector3fs[1].y(),
                vector3fs[1].z()
        ).setUv(m, n).setColor(this.rCol, this.gCol, this.bCol, this.alpha).setLight((int) p);

        vertexConsumer.addVertex(
                vector3fs[2].x(), vector3fs[2].y(),
                vector3fs[2].z()
        ).setUv(l, n).setColor(this.rCol, this.gCol, this.bCol, this.alpha).setLight((int) p);

        vertexConsumer.addVertex(
                vector3fs[3].x(), vector3fs[3].y(),
                vector3fs[3].z()
        ).setUv(l, o).setColor(this.rCol, this.gCol, this.bCol, this.alpha).setLight((int) p);
    }
