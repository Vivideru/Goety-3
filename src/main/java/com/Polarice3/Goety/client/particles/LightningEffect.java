    ...
    private static class BoltInstance {
        ...
        public void render(Matrix4f matrix, VertexConsumer buffer, Timestamp timestamp) {
            ...
            for (int i = bounds.getLeft(); i < bounds.getRight(); i++) {
                this.renderQuads.get(i).getVecs().forEach(v -> buffer.addVertex(matrix, (float) v.x, (float) v.y, (float) v.z)
                        .setColor(this.options.getColor().red(), this.options.getColor().green(), this.options.getColor().blue(), options.getColor().alpha())
                        );
            }
        }
