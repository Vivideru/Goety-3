...
public class SmashParticleOption implements ParticleOptions {
    public static final MapCodec<SmashParticleOption> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, SmashParticleOption> STREAM_CODEC = StreamCodec.of(
           (buffer, option) -> option.writeToNetwork(buffer),
           SmashParticleOption::fromNetwork
   );

   private static SmashParticleOption fromNetwork(RegistryFriendlyByteBuf buffer) {
            return new SmashParticleOption(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readInt());
   }
