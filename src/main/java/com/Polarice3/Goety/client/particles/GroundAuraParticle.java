...
public class GroundAuraParticle extends GroundCircleParticle {
    ...
    protected GroundAuraParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, SpriteSet spriteSet, int ownerId, float size, float red, float green, float blue) {
        super(level, x, y, z);
    ...
    public static class Provider implements ParticleProvider<GroundAuraParticleOption> {
        ...
        public Particle createParticle(GroundAuraParticleOption pType, ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
            return new GroundAuraParticle(pLevel, pX, pY, pZ, pXSpeed, pYSpeed, pZSpeed, this.sprites, pType.getOwnerId(), pType.getSize(), pType.getRed(), pType.getGreen(), pType.getBlue());
        }
    }
}
