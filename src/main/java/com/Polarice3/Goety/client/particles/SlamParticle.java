    ...
    public void render(VertexConsumer buffer, Camera camera, float partialTicks) {
        ...
        if (smoothAge < lifetime) {
            ...
            for (int i = 0; i < SAMPLES; i++) {
                ...
                consumer.addVertex(pose.pose(), x * smallRadius, 0, z * smallRadius).setColor(smallBaseColor);
                consumer.addVertex(pose.pose(), nextX * smallRadius, 0, nextZ * smallRadius).setColor(smallBaseColor);
                consumer.addVertex(pose.pose(), nextX * smallRadius, height, nextZ * smallRadius).setColor(topColor);
                consumer.addVertex(pose.pose(), x * smallRadius, height, z * smallRadius).setColor(topColor);

                consumer.addVertex(pose.pose(), x * radius, 0, z * radius).setColor(baseColor);
                consumer.addVertex(pose.pose(), nextX * radius, 0, nextZ * radius).setColor(baseColor);
                consumer.addVertex(pose.pose(), nextX * radius, height, nextZ * radius).setColor(topColor);
                consumer.addVertex(pose.pose(), x * radius, height, z * radius).setColor(topColor);
                ...
                if (sphereAlpha > 0) {
                    for (int j = 0; j < HEIGHT_SAMPLES; j++) {
                        ...
                        consumer.addVertex(pose.pose(), x * currentRadius, sphereHeight, z * currentRadius).setColor(sphereColor);
                        consumer.addVertex(pose.pose(), nextX * currentRadius, sphereHeight, nextZ * currentRadius).setColor(sphereColor);
                        consumer.addVertex(pose.pose(), nextX * nextRadius, nextSphereHeight, nextZ * nextRadius).setColor(sphereColor);
                        consumer.addVertex(pose.pose(), x * nextRadius, nextSphereHeight, z * nextRadius).setColor(sphereColor);
                    }
                }
            }
        }
