...
public class CircleExplodeParticleOption implements ParticleOptions {
   ...
   public static final StreamCodec<RegistryFriendlyByteBuf, CircleExplodeParticleOption> STREAM_CODEC = StreamCodec.of(
           ...
           CircleExplodeParticleOption::fromNetwork
   );

   private static CircleExplodeParticleOption fromNetwork(RegistryFriendlyByteBuf buffer) {
         return new CircleExplodeParticleOption(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readInt());
   }
